...
public class Bookmarks extends AbstractTableModel {
	...
	/**
	 * Replaces the {@link TitledURLEntry} entry at the specified index location with the new
	 * specified entry.
	 *
     * @param index index of the entry to replace
     * @param entry entry to be stored at the specified position
     * @return the entry previously at the specified position
	 */
    public TitledURLEntry set(int index, TitledURLEntry entry) {
        synchronized (entries) {
            TitledURLEntry oldEntry = entries.set(index, entry);
            fireTableDataChanged();

            //sl start
            VFSURIValidator validator = new VFSURIValidator();
            if (!validator.isValid(entry.getURL())){
                //popup a warning
                JOptionPane.showMessageDialog(null,VFSResources.getMessage("VFSFileChooser.errBADURI"));
            }
